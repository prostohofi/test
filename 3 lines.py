...
def start_combat(player_name, player_hp, player_money, player_xp, player_level, player_potions):
    ...
    while player_hp > 0 and enemy_hp > 0:
        ...
        print("- " * 20)
        enemy_hp = combat_turn(player_name, player_hp, enemy_name, enemy_hp)
        player_hp = combat_turn(enemy_name, enemy_hp, player_name, player_hp)
        pause()
    ...
    if player_hp > 0:
        ...
        next_level = player_xp // 100 + 1
        max_xp_level = 100
        if next_level > player_level : #дать новый уровень, если достаточно опыта
...
def combat_turn(attacker_name, attacker_hp, defender_name, defender_hp):
    if attacker_hp > 0:
