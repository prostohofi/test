...
allowed_passangers = 0

banned_items = ("нож, бензин, граната, пистолет")
for passenger in passengers:
    for item in passenger["ручная кладь"]:
        if item in banned_items:
                print(f"В ручной клади пассажира {passenger['имя']} обнаружен запрещённый предмет {item}. Пассажир не допущен к вылету.")
                break
        else:
            allowed_passangers += 1
            print(f"Пассажир {passenger['имя']} допущен к вылету.")   # пассажир не допущеняя

print(f"Допущено к вылету пассажиров: {allowed_passangers}")